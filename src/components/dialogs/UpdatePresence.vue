  ...
  <v-row justify="center">
    ...
    >
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row v-if="selectedAttendance.attendance">
              ...
              <v-col class="text-center pt-1" cols="10" offset-lg="1">
                <h2 class="app-heading-thin app-text-line-height">
                  Anak dengan nama {{ selectedAttendance.fullname }} sudah hadir pada
                </h2>
            ...
            <v-row v-if="!selectedAttendance.attendance && !selectedAttendance.absence">
              <v-col class="mx-auto pt-3 pb-0" cols="12 ">
                <v-img
                  contain
                  height="240"
            ...
            <v-row v-if="selectedAttendance.absence">
              <v-col class="mx-auto pl-0 pt-3 pb-0" cols="12 ">
                <v-chip color="warning">
                  <v-icon class="pl-2" size="20" left v-text="selectedAttendance.absence.absence_category == 'izin' ? 'mdi-file-document' : 'mdi-medical-bag'"></v-icon>
                  <h3 class="app-heading-thin app-text-white pr-2">{{selectedAttendance.absence.absence_category}}</h3>
                </v-chip>
                <v-row>
                  <v-col cols="6" class="mt-4 mb-4" 
                    v-for="(absenceImage, absenceImageIndex) of selectedAttendance.absence.absence_attachments"
                      ...
                      >
                          ...
                          <v-fade-transition>
                            <v-overlay
                              ...
                              color="black"
                            >
                              <v-btn text @click="zoomImage(`${apiHost}${absenceImage.attachment_url}`)"> <v-icon left>mdi-magnify</v-icon> Perbesar</v-btn>
                            </v-overlay>
                          </v-fade-transition>
                        </v-card>
                      </template>
                ...
                <v-card class="mt-1">
                  <v-card-text
                    ><p>
                      {{selectedAttendance.absence.absence_description}}
                    </p>
                  </v-card-text>
                </v-card>
              </v-col>
...
export default {
  ...
  methods: {
    ...
    presentTime(){
      let presentDate = this.selectedAttendance.attendance.present_datetime
