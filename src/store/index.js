...
export default new Vuex.Store({
  ...
  actions: {
    ...
    getAbsenceData: async (state) => {
      await state.dispatch('isLoggedIn')
      try {
        const responseStatus = await Request.GetAbsences()
        if (responseStatus.isError) {
          state.commit('setNotification', `Terjadi kesalahan saat merequest ketidak hadiran siswa, ${responseStatus.reason}`)
          return
        }
        ...
        console.warn(exception.message)
        state.commit('setNotification', `Terjadi kesalahan saat merequest ketidak hadiran siswa, ${exception.message}`)
